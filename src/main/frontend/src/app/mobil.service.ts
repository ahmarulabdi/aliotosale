...
export class MobilService {
  ...
  getMobilByBrand(brand: string): Observable<EntityArrayResponseMobil> {
    return this.http.get<Mobil[]>(`${this.mobilUrl}/${brand}/merk`, {observe: 'response'});
  }

  getMobilByTypeNama(typeNama: string): Observable<EntityArrayResponseMobil> {
    return this.http.get<Mobil[]>(`${this.mobilUrl}/${typeNama}/type`, {observe: 'response'});
  }
